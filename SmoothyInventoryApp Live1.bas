...
Public Function MatchCol(ws As Worksheet, ByVal headerText As Variant) As Long
    Dim m As Variant
    On Error Resume Next
...
Public Function MatchConsoleColAny(ws As Worksheet) As Long
    Dim c As Long
...
Private Function NormalizeBusiness(ByVal s As String) As String
    ...
    If t = LCase$(BIZ_VG) Then NormalizeBusiness = BIZ_VG: Exit Function
    If t = LCase$(BIZ_PK) Then NormalizeBusiness = BIZ_PK: Exit Function
    If t = LCase$(BIZ_CL) Then NormalizeBusiness = BIZ_CL: Exit Function
    If t = LCase$(BIZ_ALL) Then NormalizeBusiness = BIZ_ALL: Exit Function

    ' SKU prefix inference (optional)
    ...
    ' Unknown -> return blank (do NOT force VG)
    NormalizeBusiness = ""
End Function
...
Private Function ActiveBusinessName() As String
    ...
    Dim nb As String
    nb = NormalizeBusiness(v)
    If nb = "" Then nb = BIZ_ALL
    ActiveBusinessName = nb
End Function
...
End Sub
Public Sub HighlightZeroQtyInventory()
    ...
    If qtyCol = 0 Then
        MsgBox "Couldn't find a 'Quantity' column in Inventory row 1.", vbExclamation, MSG_TITLE
        Exit Sub
    End If
    ...
    ' Clear previous row fills (rows 2..lastRow, all used columns)
    wsInv.Range(wsInv.Cells(2, 1), wsInv.Cells(lastRow, lastCol)).Interior.Pattern = xlNone
    wsInv.Range(wsInv.Cells(2, 1), wsInv.Cells(lastRow, lastCol)).Font.ColorIndex = xlAutomatic

    ' Apply red highlight where Quantity = 0
    For r = 2 To lastRow
        If Val(wsInv.Cells(r, qtyCol).value) = 0 Then
            wsInv.Range(wsInv.Cells(r, 1), wsInv.Cells(r, lastCol)).Interior.Color = RGB(255, 199, 206) ' light red
            wsInv.Cells(r, qtyCol).Font.Color = RGB(156, 0, 6)
        End If
    Next r
End Sub
...
' =====================================================================================
Public Function AddStockField(ws As Worksheet, header As String) As Range
    Dim f As Range
    On Error Resume Next
...
Public Sub ClearAddStockForm()
    ...
    BindAddStockQualityDropdown ws
End Sub
Public Sub BindInventoryConsoleDropdownRow(ws As Worksheet, ByVal rowNum As Long)
    Dim bizCol As Long, consoleCol As Long
    Dim bizVal As String, formulaToUse As String

    bizCol = MatchCol(ws, "Business")
    consoleCol = MatchConsoleColAny(ws)
    If bizCol = 0 Or consoleCol = 0 Then Exit Sub
    If rowNum < 2 Then Exit Sub

    EnsureSettingsBasics

    bizVal = NormalizeBusiness(CStr(ws.Cells(rowNum, bizCol).value))
    If bizVal = "" Or bizVal = BIZ_ALL Then bizVal = BIZ_VG

    formulaToUse = BusinessConsoleNamedList(bizVal)

    With ws.Cells(rowNum, consoleCol).Validation
        .Delete
...
End Sub
' =====================================================================================
' FINANCE (Sales + Expenses + Monthly Summary)
' =====================================================================================
Private Function FinanceSalesHeaders() As Variant
    FinanceSalesHeaders = Array( _
        "Timestamp", "Sale Date", "Business", "SKU", "Console / Product", "Game Name", "Quality", "Country", _
        "Qty", "Gross", "Fees", "Shipping", "COGS", "Profit", "Notes" _
    )
End Function

Private Function FinanceExpenseHeaders() As Variant
    FinanceExpenseHeaders = Array("Timestamp", "Date", "Business", "Category", "Description", "Amount", "Notes")
End Function

Private Sub EnsureFinanceSheet()
    Dim ws As Worksheet
    Dim i As Long
    Dim hdr As Variant

    Set ws = EnsureSheet("Finance")
    ApplySheetChrome ws, False

    ' Layout:
    ' A1:O1 = Sales headers
    ' Q1:W1 = Expense headers (start at column Q)
    If Trim$(CStr(ws.Cells(1, 1).value)) = "" Then
        hdr = FinanceSalesHeaders()
        For i = LBound(hdr) To UBound(hdr)
            ws.Cells(1, i + 1).value = hdr(i)
        Next i
    End If

    If Trim$(CStr(ws.Cells(1, 17).value)) = "" Then ' Q = 17
        hdr = FinanceExpenseHeaders()
        For i = LBound(hdr) To UBound(hdr)
            ws.Cells(1, 17 + i).value = hdr(i)
        Next i
    End If
    ...
    ws.Columns("Q:W").AutoFit

    ' NEW: ensure Finance has its button
    EnsureFinanceBusinessSelector
    SetupFinanceButtons
    RebindFinanceExpenseCategoryDropdown
End Sub
...
Private Sub AppendFinanceSale( _
    ...
    ByVal bizVal As String, _
    ByVal sku As String, _
    ByVal consoleVal As String, _
    ByVal gameNameVal As String, _
    ByVal qualityVal As String, _
    ByVal countryVal As String, _
    ByVal qty As Long, _
    ByVal gross As Double, _
    ByVal fees As Double, _
    ByVal shipping As Double, _
    ByVal cogs As Double, _
    ...
    ws.Cells(r, "B").value = saleDate
    ws.Cells(r, "C").value = bizVal
    ws.Cells(r, "D").value = sku
    ws.Cells(r, "E").value = consoleVal
    ws.Cells(r, "F").value = gameNameVal
    ws.Cells(r, "G").value = qualityVal
    ws.Cells(r, "H").value = countryVal
    ws.Cells(r, "I").value = qty
    ws.Cells(r, "J").value = gross
    ws.Cells(r, "K").value = fees
    ws.Cells(r, "L").value = shipping
    ws.Cells(r, "M").value = cogs
    ws.Cells(r, "N").value = profit
    ws.Cells(r, "O").value = notes

    ws.Columns("B").NumberFormat = "m/d/yyyy"
    ws.Columns("J:N").NumberFormat = "$#,##0.00"
End Sub

Public Sub RefreshFinance(): SafeRun "Refresh Finance", "RefreshFinance_Worker": End Sub

Private Sub RefreshFinance_Worker()
    EnsureThemeReady
    EnsureFinanceSheet

    Dim ws As Worksheet: Set ws = EnsureSheet("Finance")
    Dim viewBiz As String
    viewBiz = ActiveFinanceViewBusiness()
    Dim lastRow As Long: lastRow = ws.Cells(ws.Rows.Count, "A").End(xlUp).Row   ' Timestamp column for sales is safest
    If lastRow < 2 Then
        MsgBox "No sales yet.", vbInformation, MSG_TITLE
        Exit Sub
    End If

    ' Build monthly summary on the right so it never overwrites sales/expenses
    Dim outTop As Long: outTop = 1

    ' Clear + header
    ws.Range("AA" & outTop & ":AH2000").ClearContents
    ws.Range("AA" & outTop & ":AH" & outTop).value = Array( _
        "Month", "Income", "Fees", "Shipping", "COGS", "Other Exp", "Profit", "Net (after exp)" _
    )
    ws.Range("AA" & outTop & ":AH" & outTop).Font.Bold = True
    ws.Range("AA" & outTop & ":AH" & outTop).Interior.Color = CLR_PANEL

    Dim d As Object: Set d = CreateObject("Scripting.Dictionary")
    Dim r As Long, key As String, dt As Date
    Dim gross As Double, fees As Double, ship As Double, cogs As Double, prof As Double
    Dim a As Variant

    ' Sales monthly (Sale Date in col B; amounts in J:N)
   For r = 2 To lastRow
    If IsDate(ws.Cells(r, "B").value) Then

        ' Business filter (Sales business is column C)
        If viewBiz <> BIZ_ALL Then
            If NormalizeBusiness(CStr(ws.Cells(r, "C").value)) <> viewBiz Then GoTo NextSaleRow
        End If

        dt = CDate(ws.Cells(r, "B").value)
        key = Format$(DateSerial(Year(dt), Month(dt), 1), "yyyy-mm")

        gross = Val(ws.Cells(r, "J").value)
        fees = Val(ws.Cells(r, "K").value)
        ship = Val(ws.Cells(r, "L").value)
        cogs = Val(ws.Cells(r, "M").value)
        prof = Val(ws.Cells(r, "N").value)

        If Not d.Exists(key) Then d.Add key, Array(0#, 0#, 0#, 0#, 0#)
        a = d(key)
        a(0) = a(0) + gross
        a(1) = a(1) + fees
        a(2) = a(2) + ship
...
Next r
    ...
    ' Expenses monthly (from Q:W) => Date in R, Amount in U
    Dim lastExp As Long: lastExp = ws.Cells(ws.Rows.Count, "V").End(xlUp).Row
    Dim expDict As Object: Set expDict = CreateObject("Scripting.Dictionary")
    Dim expAmt As Double

    If lastExp >= 2 Then
        For r = 2 To lastExp
    If IsDate(ws.Cells(r, "R").value) Then

        ' Business filter (Expense business is column S)
        If viewBiz <> BIZ_ALL Then
            If NormalizeBusiness(CStr(ws.Cells(r, "S").value)) <> viewBiz Then GoTo NextExpRow
        End If

        dt = CDate(ws.Cells(r, "R").value)
        key = Format$(DateSerial(Year(dt), Month(dt), 1), "yyyy-mm")
        expAmt = Val(ws.Cells(r, "V").value)

        If Not expDict.Exists(key) Then expDict.Add key, 0#
        expDict(key) = expDict(key) + expAmt
    End If
NextExpRow:
Next r
    End If

    ' Nothing to summarize (edge case)
    If d.Count = 0 Then
        MsgBox "No valid dated sales rows found (check Sale Date column).", vbExclamation, MSG_TITLE
        Exit Sub
    End If

    ' Sort keys
    Dim keys() As String, i As Long, j As Long, tmp As String
    ReDim keys(0 To d.Count - 1)
    i = 0
    Dim kk As Variant
    ...
    ' Output
    Dim outRow As Long: outRow = outTop + 1
    Dim otherExp As Double, netAfterExp As Double
    ...
    For i = LBound(keys) To UBound(keys)
        ...
        otherExp = 0#
        If expDict.Exists(key) Then otherExp = expDict(key)

        netAfterExp = a(4) - otherExp

        ws.Range("AA" & outRow).value = key
        ws.Range("AB" & outRow).value = a(0)
        ws.Range("AC" & outRow).value = a(1)
        ws.Range("AD" & outRow).value = a(2)
        ws.Range("AE" & outRow).value = a(3)
        ws.Range("AF" & outRow).value = otherExp
        ws.Range("AG" & outRow).value = a(4)
        ws.Range("AH" & outRow).value = netAfterExp

        ws.Range("AB" & outRow & ":AH" & outRow).NumberFormat = "$#,##0.00"
        If outRow Mod 2 = 0 Then ws.Range("AA" & outRow & ":AH" & outRow).Interior.Color = CLR_ALTROW
        outRow = outRow + 1
    Next i

    ws.Columns("AA:AH").AutoFit
    'MsgBox "Finance refreshed.", vbInformation, MSG_TITLE
End Sub
...
Public Sub BuildFilteredFinanceViews()
    ...
    Dim ws As Worksheet: Set ws = EnsureSheet("Finance")
    Dim viewBiz As String: viewBiz = ActiveFinanceViewBusiness()

    Application.ScreenUpdating = False

    ' =========================
    ' FILTERED SALES VIEW (AI:AW)
    ' =========================
    Dim salesHeaders As Variant
    salesHeaders = FinanceSalesHeaders()

    ws.Range("AI:AW").ClearContents

    ws.Range("AI1").value = "Filtered Sales (" & viewBiz & ")"
    ws.Range("AI1").Font.Bold = True

    ws.Range("AI2").Resize(1, UBound(salesHeaders) + 1).value = salesHeaders
    ws.Range("AI2").Resize(1, UBound(salesHeaders) + 1).Font.Bold = True
    ws.Range("AI2").Resize(1, UBound(salesHeaders) + 1).Interior.Color = CLR_HEADER

    Dim srcLast As Long, r As Long, outR As Long
    srcLast = ws.Cells(ws.Rows.Count, "D").End(xlUp).Row   ' SKU column is more likely filled
    If srcLast < 2 Then srcLast = 2

    outR = 3
    For r = 2 To srcLast
    ' Consider it a row if it has any meaningful sales data
    Dim hasSale As Boolean
    hasSale = (Trim$(CStr(ws.Cells(r, "B").value)) <> "" Or _
               Trim$(CStr(ws.Cells(r, "D").value)) <> "" Or _
               Trim$(CStr(ws.Cells(r, "J").value)) <> "")

    If hasSale Then
        If viewBiz = BIZ_ALL Or NormalizeBusiness(CStr(ws.Cells(r, "C").value)) = viewBiz Then
            ws.Range("AI" & outR).Resize(1, 15).value = ws.Range("A" & r).Resize(1, 15).value
            outR = outR + 1
        End If
    End If
Next r

    ws.Range("AJ:AJ").NumberFormat = "m/d/yyyy"
    ws.Range("AR:AV").NumberFormat = "$#,##0.00"  ' Gross..COGS in filtered view (J:M maps to AR:AU but keep simple)
    ws.Columns("AI:AW").AutoFit

    ' =========================
    ' FILTERED EXPENSES VIEW (AY:BE)
    ' =========================
    Dim expHeaders As Variant
    expHeaders = FinanceExpenseHeaders()

    ws.Range("AY:BE").ClearContents

    ws.Range("AY1").value = "Filtered Expenses (" & viewBiz & ")"
    ws.Range("AY1").Font.Bold = True

    ws.Range("AY2").Resize(1, UBound(expHeaders) + 1).value = expHeaders
    ws.Range("AY2").Resize(1, UBound(expHeaders) + 1).Font.Bold = True
    ws.Range("AY2").Resize(1, UBound(expHeaders) + 1).Interior.Color = CLR_HEADER

    Dim expLast As Long
    expLast = ws.Cells(ws.Rows.Count, "V").End(xlUp).Row   ' Amount column is more likely filled
    If expLast < 2 Then expLast = 2

    outR = 3
    For r = 2 To expLast
    ' Consider it a row if it has any meaningful expense data
    Dim hasExp As Boolean
    hasExp = (Trim$(CStr(ws.Cells(r, "R").value)) <> "" Or _
              Trim$(CStr(ws.Cells(r, "U").value)) <> "" Or _
              Trim$(CStr(ws.Cells(r, "V").value)) <> "")

    If hasExp Then
        If viewBiz = BIZ_ALL Or NormalizeBusiness(CStr(ws.Cells(r, "S").value)) = viewBiz Then
            ws.Range("AY" & outR).Resize(1, 7).value = ws.Range("Q" & r).Resize(1, 7).value
            outR = outR + 1
        End If
    End If
Next r

    ws.Range("AZ:AZ").NumberFormat = "m/d/yyyy"
    ws.Range("BD:BD").NumberFormat = "$#,##0.00"
    ws.Columns("AY:BE").AutoFit

    Application.ScreenUpdating = True
End Sub
Public Sub FinanceResetLeftView()
    EnsureThemeReady
    EnsureFinanceSheet
    ...
    ' Unhide everything (in case an older version hid rows)
    ws.Rows.Hidden = False

    ' Remove any AutoFilters that might still be active from previous attempts
    On Error Resume Next
    ...
    MsgBox "Finance reset: left-side tables restored (no filters, no hidden rows).", vbInformation, MSG_TITLE
End Sub
Public Sub FinanceApplyFilters()
    ' No hiding / no AutoFilter: just rebuild the right-side views + refresh totals
    BuildFilteredFinanceViews
    RefreshFinance
End Sub
Public Sub AddFinanceExpense()
    EnsureThemeReady
    EnsureFinanceSheet
    ...
    r = ws.Cells(ws.Rows.Count, "Q").End(xlUp).Row + 1
    If r < 2 Then r = 2

    Dim expDateInput As String, expDate As Date
    Dim bizVal As String, cat As String, descr As String, notes As String
    Dim amtInput As String, amt As Double

    expDateInput = Trim$(InputBox("Expense date (blank = today). Format: dd/mm/yyyy", MSG_TITLE, ""))
If expDateInput = "" Then
    expDate = Date
Else
    expDate = ParseDate_DDMMYYYY(expDateInput)
    If expDate = 0 Then
        MsgBox "Invalid date. Please use dd/mm/yyyy. Using today.", vbExclamation, MSG_TITLE
        expDate = Date
    End If
End If

   Dim bizRaw As String
bizRaw = Trim$(InputBox( _
    "Business shortcuts:" & vbCrLf & _
    "  VG = Video Games" & vbCrLf & _
    "  PK = Pokemon" & vbCrLf & _
    "  CG = Cologne" & vbCrLf & vbCrLf & _
    "Enter Business (blank = Active Business):", _
    MSG_TITLE, _
    ActiveBusinessName() _
))

bizVal = ParseBusinessShortcut(bizRaw, ActiveBusinessName())

    cat = Trim$(InputBox("Category (choose or type one):" & vbCrLf & vbCrLf & GetExpenseCategoryPromptList(), MSG_TITLE, ""))
    descr = Trim$(InputBox("Description:", MSG_TITLE, ""))
    amtInput = Trim$(InputBox("Amount (positive number):", MSG_TITLE, "0"))
    amt = Val(amtInput)

    If amt <= 0 Then
        MsgBox "Amount must be > 0. Expense not added.", vbExclamation, MSG_TITLE
        Exit Sub
    End If

    notes = Trim$(InputBox("Notes (optional):", MSG_TITLE, ""))

    ws.Cells(r, "Q").value = Now
    ws.Cells(r, "R").value = expDate
    ws.Cells(r, "S").value = bizVal
    ws.Cells(r, "T").value = cat
    ws.Cells(r, "U").value = descr
    ws.Cells(r, "V").value = amt
    ws.Cells(r, "W").value = notes
    ...
    RefreshFinance
End Sub
Private Function GetExpenseCategoryPromptList() As String
    On Error GoTo Fallback

    Dim nm As Name
    Set nm = ThisWorkbook.names("ExpenseCategoryList")
    ...
    For Each c In rng.Cells
        If Trim$(CStr(c.value)) <> "" Then
            s = s & " - " & Trim$(CStr(c.value)) & vbCrLf
        End If
    Next c

    If s <> "" Then
        GetExpenseCategoryPromptList = "Available Categories:" & vbCrLf & s
        Exit Function
    End If

Fallback:
    GetExpenseCategoryPromptList = "(No category list found yet. Run EnsureSettingsBasics / SetupMultiBusinessSupport.)"
End Function
Public Sub SetupFinanceButtons()
    Dim ws As Worksheet
    Set ws = EnsureSheet("Finance")
    EnsureThemeReady

    On Error Resume Next
    ws.Shapes("btn_Finance_Refresh").Delete
    ws.Shapes("btn_Finance_AddExpense").Delete
    ws.Shapes("btn_Finance_FilterAll").Delete
    ws.Shapes("btn_Finance_FilterVG").Delete
    ws.Shapes("btn_Finance_FilterPK").Delete
    ws.Shapes("btn_Finance_FilterCL").Delete
    On Error GoTo 0

    Dim leftPos As Double, topPos As Double
    Dim btnW As Double, btnH As Double, gap As Double

    leftPos = ws.Columns("X").left + 2
    topPos = ws.Rows(6).top + 2   ' <-- START LOWER so selector at Y4/Z4 is always visible

    btnW = (ws.Columns("X").width + ws.Columns("Y").width) - 6
    btnH = 26
    gap = 6

    AddAppButton ws, "Refresh Finance", "RefreshFinance", leftPos, topPos, btnW, btnH, "btn_Finance_Refresh"
    AddAppButton ws, "Add Expense", "AddFinanceExpense", leftPos, topPos + (btnH + gap) * 1, btnW, btnH, "btn_Finance_AddExpense"

    AddAppButton ws, "Show: All", "FinanceFilter_All", leftPos, topPos + (btnH + gap) * 2, btnW, btnH, "btn_Finance_FilterAll"
    AddAppButton ws, "Show: Video Games", "FinanceFilter_VideoGames", leftPos, topPos + (btnH + gap) * 3, btnW, btnH, "btn_Finance_FilterVG"
    AddAppButton ws, "Show: Pokemon", "FinanceFilter_Pokemon", leftPos, topPos + (btnH + gap) * 4, btnW, btnH, "btn_Finance_FilterPK"
    AddAppButton ws, "Show: Cologne", "FinanceFilter_Cologne", leftPos, topPos + (btnH + gap) * 5, btnW, btnH, "btn_Finance_FilterCL"

    RecolorButtons ws
End Sub
Private Function ParseBusinessShortcut(ByVal s As String, Optional ByVal defaultBiz As String = "") As String
    Dim t As String
    t = UCase$(Trim$(s))

    Select Case t
        Case "", "DEFAULT"
            ParseBusinessShortcut = NormalizeBusiness(defaultBiz)
        Case "VG", "VIDEO", "VIDEOGAMES", "VIDEO GAMES", "GAMES"
            ParseBusinessShortcut = BIZ_VG
        Case "PK", "POKEMON", "POKE"
            ParseBusinessShortcut = BIZ_PK
        Case "CG", "COLOGNE", "COLOGN"
            ParseBusinessShortcut = BIZ_CL
        Case "ALL", "A"
            ParseBusinessShortcut = BIZ_ALL
        Case Else
            ' Allow full names too
            ParseBusinessShortcut = NormalizeBusiness(s)
    End Select

    If ParseBusinessShortcut = "" Then ParseBusinessShortcut = NormalizeBusiness(defaultBiz)
    If ParseBusinessShortcut = "" Then ParseBusinessShortcut = BIZ_ALL
End Function
...
End Function
Private Sub EnsureExpenseCategoryNamedRange()
    Dim ws As Worksheet
    Dim lastRow As Long

    Set ws = EnsureSheet("Settings")

    ' Put categories in Settings column AA (out of the way)
    If Trim$(CStr(ws.Range("AA1").value)) = "" Then
        ws.Range("AA1").value = "Expense Categories"
        ws.Range("AA2").value = "Shipping Supplies"
        ws.Range("AA3").value = "Postage"
        ws.Range("AA4").value = "Gas"
        ws.Range("AA5").value = "Subscriptions"
        ws.Range("AA6").value = "Repairs"
        ws.Range("AA7").value = "Other"
    End If
    ...
    On Error Resume Next
    ThisWorkbook.names("ExpenseCategoryList").Delete
    On Error GoTo 0

    ThisWorkbook.names.Add Name:="ExpenseCategoryList", _
        RefersTo:="='" & ws.Name & "'!" & ws.Range("AA2:AA" & lastRow).Address
End Sub
Private Sub AddOrIncrementBarcode(wsBar As Worksheet, ByVal bc As String, ByVal sku As String)
    ...
    End If
End Sub
Private Sub BindBatchGenericDropdown(ws As Worksheet, ByVal headerName As String, ByVal formulaToUse As String)
    Dim tgtCol As Long, r As Long
    tgtCol = MatchCol(ws, headerName)
    If tgtCol = 0 Then Exit Sub

    For r = 2 To 300
        With ws.Cells(r, tgtCol).Validation
            .Delete
            .Add xlValidateList, xlValidAlertStop, xlBetween, formulaToUse
        End With
    Next r
...
Private Sub BindBatchPerBizDropdown(ws As Worksheet, ByVal headerName As String, ByVal listFnName As String)
    Dim bizCol As Long, tgtCol As Long, r As Long
    Dim bizVal As String, formulaToUse As String

    bizCol = MatchCol(ws, "Business")
    tgtCol = MatchCol(ws, headerName)
    If bizCol = 0 Or tgtCol = 0 Then Exit Sub

    For r = 2 To 300
        bizVal = NormalizeBusiness(CStr(ws.Cells(r, bizCol).value))
        If bizVal = "" Or bizVal = BIZ_ALL Then bizVal = BIZ_VG

        Select Case LCase$(listFnName)
            Case "from":          formulaToUse = BusinessFromNamedList(bizVal)
            Case "listed_status": formulaToUse = BusinessListedStatusNamedList(bizVal)
            Case "stored":        formulaToUse = BusinessStoredNamedList(bizVal)
            Case "tested":        formulaToUse = BusinessTestedNamedList(bizVal)
            Case "bundle_number": formulaToUse = BusinessBundleNamedList(bizVal)
            Case "country":       formulaToUse = BusinessCountryNamedList(bizVal)
            Case Else:            formulaToUse = ""
        End Select

        If formulaToUse <> "" Then
            With ws.Cells(r, tgtCol).Validation
                .Delete
                .Add xlValidateList, xlValidAlertStop, xlBetween, formulaToUse
            End With
...
Public Sub BindBatchOtherDropdowns(ws As Worksheet)
    ' These header strings must match BatchEntryHeaders() exactly:
    BindBatchPerBizDropdown ws, "From", "from"
    BindBatchPerBizDropdown ws, "Listed Status", "listed_status"
    BindBatchPerBizDropdown ws, "Stored", "stored"
    BindBatchPerBizDropdown ws, "Tested", "tested"
    BindBatchPerBizDropdown ws, "Bundle Number", "bundle_number"
    BindBatchPerBizDropdown ws, "Country", "country"
End Sub
...
Private Sub RemoveStock_Single_Worker()
    ...
    Dim feesInput As String, shipInput As String
    Dim fees As Double, shipping As Double
    Dim saleDateInput As String, saleDate As Date
    Dim financeNotes As String
    ...
    If markSold = vbYes Then
        ...
        saleDateInput = Trim$(InputBox("Sale date (optional). Leave blank for today." & vbCrLf & "Format: mm/dd/yyyy", MSG_TITLE, ""))
        If saleDateInput = "" Then
            saleDate = Date
        ElseIf IsDate(saleDateInput) Then
            saleDate = CDate(saleDateInput)
        Else
            MsgBox "Invalid date. Using today.", vbExclamation, MSG_TITLE
            saleDate = Date
        End If
        ...
        salePriceInput = InputBox( _
            "Gross sale amount (TOTAL for these " & qtyToProcess & " unit(s))." & vbCrLf & _
            "(Leave blank to record without price)", _
            MSG_TITLE, _
            IIf(suggestedTotal > 0, CStr(suggestedTotal), "") _
        )
        If Trim$(salePriceInput) <> "" Then salePrice = Val(salePriceInput)

        ' Fees + Shipping (only meaningful if salePrice provided)
        feesInput = InputBox("Platform/Payment fees (TOTAL):", MSG_TITLE, "0")
        shipInput = InputBox("Shipping cost (TOTAL):", MSG_TITLE, "0")
        fees = Val(feesInput)
        shipping = Val(shipInput)

        financeNotes = Trim$(InputBox("Notes (optional):", MSG_TITLE, ""))
        ...
        If salePrice > 0 Then
          totalCost = Val(wsInv.Cells(foundCell.Row, cols.paid).value) * qtyToProcess ' COGS
            realizedProfit = salePrice - fees - shipping - totalCost
        ...
        LogHistory_AddRemove "Sold", wsInv, wsHist, foundCell.Row, -qtyToProcess, "", salePrice
         ' NEW: append to Finance sheet (only if sale price entered)
        If salePrice > 0 Then
            AppendFinanceSale _
                saleDate, _
                GetRowBusiness(wsInv, foundCell.Row, cols, True), _
                CStr(wsInv.Cells(foundCell.Row, cols.sku).value), _
                CStr(wsInv.Cells(foundCell.Row, cols.Console).value), _
                CStr(wsInv.Cells(foundCell.Row, cols.GameName).value), _
                CStr(wsInv.Cells(foundCell.Row, cols.Quality).value), _
                CStr(wsInv.Cells(foundCell.Row, cols.Country).value), _
                qtyToProcess, _
                salePrice, _
                fees, _
                shipping, _
...
For i = LBound(headers) To UBound(headers)
    Dim c As Long
    c = MatchCol(wsInv, headers(i))

    If c > 0 Then
        wsHist.Cells(r, 9 + i).value = wsInv.Cells(invRow, c).value
    Else
        wsHist.Cells(r, 9 + i).value = ""
    End If
Next i
    ...
    End Select
End Sub
Public Sub BindInventoryConsoleDropdowns(ws As Worksheet)
    Dim bizCol As Long, consoleCol As Long, r As Long, lastRow As Long
    Dim bizVal As String, formulaToUse As String

    bizCol = MatchCol(ws, "Business")
    consoleCol = MatchConsoleColAny(ws) ' supports "Console" or "Console / Product"
    If bizCol = 0 Or consoleCol = 0 Then Exit Sub

    lastRow = ws.Cells(ws.Rows.Count, bizCol).End(xlUp).Row
    If lastRow < 2 Then lastRow = 2
    ...
    For r = 2 To lastRow
        ...
        With ws.Cells(r, consoleCol).Validation
            .Delete
            .Add xlValidateList, xlValidAlertStop, xlBetween, formulaToUse
        End With
    Next r
...
Private Sub PerformSellFromSKU(ByVal sku As String)
    ...
    Dim saleGross As Double, fees As Double, shipping As Double
    Dim totalCost As Double, realizedProfit As Double, perUnit As Double
    Dim saleDateInput As String, saleDate As Date
    Dim notes As String
    ...
    ' Gross sale total
    saleGross = Val(InputBox("Gross sale amount (TOTAL for these " & qtyToSell & "):", MSG_TITLE, _
                    Val(wsInv.Cells(rInv.Row, cols.ListedPrice).value) * qtyToSell))

    ' Fees + Shipping
    fees = Val(InputBox("Platform/Payment fees (TOTAL):", MSG_TITLE, 0))
    shipping = Val(InputBox("Shipping cost (TOTAL):", MSG_TITLE, 0))
    ...
    totalCost = Val(wsInv.Cells(rInv.Row, cols.paid).value) * qtyToSell ' COGS
    realizedProfit = saleGross - fees - shipping - totalCost
    If qtyToSell > 0 Then perUnit = saleGross / qtyToSell

    If cols.notes > 0 And saleGross > 0 Then
        wsInv.Cells(rInv.Row, cols.notes).value = AggregateSaleNotes(CStr(wsInv.Cells(rInv.Row, cols.notes).value), qtyToSell, saleGross, realizedProfit)
    End If

    ' Keep Stock History logging (sale price is gross)
    LogHistory_AddRemove "Sold", wsInv, wsHist, rInv.Row, -qtyToSell, "", saleGross
    DecrementBarcodeCountsForSKU sku, qtyToSell
    RefreshDashboard_Worker
    RepairInventoryColors

    ' NEW: append to Finance sheet
    AppendFinanceSale _
        saleDate, _
        GetRowBusiness(wsInv, rInv.Row, cols, True), _
        sku, _
        CStr(wsInv.Cells(rInv.Row, cols.Console).value), _
        CStr(wsInv.Cells(rInv.Row, cols.GameName).value), _
        CStr(wsInv.Cells(rInv.Row, cols.Quality).value), _
        CStr(wsInv.Cells(rInv.Row, cols.Country).value), _
        qtyToSell, _
        saleGross, _
        fees, _
        shipping, _
    ...
    MsgBox "Sold " & qtyToSell & " of " & sku & vbCrLf & _
           "Date: " & Format$(saleDate, "m/d/yyyy") & vbCrLf & _
           "Per Unit (gross): " & FormatCurrency(perUnit) & vbCrLf & _
           "Gross: " & FormatCurrency(saleGross) & vbCrLf & _
           "Fees: " & FormatCurrency(fees) & vbCrLf & _
           "Shipping: " & FormatCurrency(shipping) & vbCrLf & _
           "COGS: " & FormatCurrency(totalCost) & vbCrLf & _
...
Private Sub CommitBatchEntry_Worker()
    ...
    For r = 2 To lastRow
        ...
        invRow = CommitOneBatchRow(wsBatch, r, wsInv, cols, sku, qty, bizVal)
            BindInventoryConsoleDropdownRow wsInv, invRow
...
Private Function CommitOneBatchRow(wsBatch As Worksheet, ByVal batchRow As Long, wsInv As Worksheet, ByRef cols As InventoryCols, ByVal sku As String, ByVal qty As Long, ByVal bizVal As String) As Long
    ...
    If sku <> "" Then
        ...
        End If
    End If
' If SKU not provided, try to stack onto an existing identical item (identity fields only)
If Trim$(sku) = "" Then
    Dim bConsole As Long, bGame As Long, bQuality As Long, bCountry As Long
    Dim existingRow As Long

    bConsole = MatchConsoleColAny(wsBatch) ' supports "Console" or "Console / Product"
    bGame = MatchCol(wsBatch, "Game Name")
    bQuality = MatchCol(wsBatch, "Quality")
    bCountry = MatchCol(wsBatch, "Country")

    existingRow = FindMatchingInventoryRow_IdentityOnly( _
        wsInv, cols, bizVal, _
        IIf(bConsole > 0, CStr(wsBatch.Cells(batchRow, bConsole).value), ""), _
        IIf(bGame > 0, CStr(wsBatch.Cells(batchRow, bGame).value), ""), _
        IIf(bCountry > 0, CStr(wsBatch.Cells(batchRow, bCountry).value), ""), _
        IIf(bQuality > 0, CStr(wsBatch.Cells(batchRow, bQuality).value), "") _
    )

    If existingRow > 0 Then
        wsInv.Cells(existingRow, cols.Quantity).value = Val(wsInv.Cells(existingRow, cols.Quantity).value) + qty
        CommitOneBatchRow = existingRow
        Exit Function
    End If
End If
    ' Create row
...
For Each h In InventoryHeaders()
    ...
    ' ----- Special case: Console field (Inventory may be "Console" while headers say "Console / Product") -----
    If CStr(h) = "Console / Product" Then
        invCol = MatchConsoleColAny(wsInv)
        batchCol = MatchCol(wsBatch, "Console / Product")
        If batchCol = 0 Then batchCol = MatchCol(wsBatch, "Console")
...
End Sub
Public Function FindMatchingInventoryRow_IdentityOnly( _
    wsInv As Worksheet, ByRef cols As InventoryCols, _
    ByVal bizVal As String, _
    ByVal consoleVal As String, _
    ByVal gameNameVal As String, _
    ByVal countryVal As String, _
    ...
    For r = 2 To lastRow
        If NormalizeBusiness(CStr(wsInv.Cells(r, cols.Business).value)) = NormalizeBusiness(bizVal) _
           And Trim$(CStr(wsInv.Cells(r, cols.Console).value)) = Trim$(consoleVal) _
           And Trim$(CStr(wsInv.Cells(r, cols.GameName).value)) = Trim$(gameNameVal) _
           And Trim$(CStr(wsInv.Cells(r, cols.Country).value)) = Trim$(countryVal) _
           And Trim$(CStr(wsInv.Cells(r, cols.Quality).value)) = Trim$(qualityVal) Then

            FindMatchingInventoryRow_IdentityOnly = r
            Exit Function
        End If
    Next r
End Function
...
Public Sub EnsureSettingsBasics()
    Dim ws As Worksheet
    Set ws = EnsureSheet("Settings")
    ApplySheetChrome ws, False
    ...
    EnsureExpenseCategoryNamedRange
    
    ws.Rows(1).Font.Bold = True
    ws.Rows(1).Interior.Color = CLR_HEADER
End Sub
Private Sub RebindFinanceExpenseCategoryDropdown()
    EnsureSettingsBasics

    Dim ws As Worksheet: Set ws = EnsureSheet("Finance")
    Dim r As Long

    ' Category column is T (Q Timestamp, R Date, S Business, T Category)
    For r = 2 To 2000
        With ws.Cells(r, "T").Validation
            .Delete
            .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Operator:=xlBetween, Formula1:="=ExpenseCategoryList"
        End With
    Next r
End Sub
...
Public Sub BindAddStockConsoleDropdown(ws As Worksheet)
    Dim bizVal As String, formulaToUse As String
...
Public Sub BindAddStockQualityDropdown(ws As Worksheet)
    Dim bizVal As String, formulaToUse As String
    Dim tgt As Range
...
Private Sub BindBatchDropdowns_All(ws As Worksheet)
    ...
    BindBatchOtherDropdowns ws
End Sub
' ===== Batch Entry: extra per-business dropdowns =====

Private Function BusinessFieldNamedList(ByVal bizVal As String, ByVal fieldKey As String) As String
    ' fieldKey examples:
    ' "country", "from", "listed_status", "tested", "bundle_number", "stored"
    Dim b As String
    b = NormalizeBusiness(bizVal)

    Select Case b
        Case BIZ_PK: BusinessFieldNamedList = "=Field_" & fieldKey & "_PokemonList"
        Case BIZ_CL: BusinessFieldNamedList = "=Field_" & fieldKey & "_CologneList"
        Case Else:   BusinessFieldNamedList = "=Field_" & fieldKey & "_Video_GamesList"
    End Select
End Function

Private Sub BindBatchFieldDropdowns(ws As Worksheet, ByVal headerName As String, ByVal fieldKey As String)
    Dim bizCol As Long, fieldCol As Long, r As Long
    Dim bizVal As String, formulaToUse As String

    bizCol = MatchCol(ws, "Business")
    fieldCol = MatchCol(ws, headerName)
    If bizCol = 0 Or fieldCol = 0 Then Exit Sub

    For r = 2 To 300
        bizVal = NormalizeBusiness(CStr(ws.Cells(r, bizCol).value))
        If bizVal = "" Or bizVal = BIZ_ALL Then bizVal = BIZ_VG

        formulaToUse = BusinessFieldNamedList(bizVal, fieldKey)
        With ws.Cells(r, fieldCol).Validation
            .Delete
            .Add xlValidateList, xlValidAlertStop, xlBetween, formulaToUse
        End With
    Next r
...
Public Sub BindBatchExtraDropdowns(ws As Worksheet)
    ' These header names must match your Batch Entry headers exactly:
    BindBatchFieldDropdowns ws, "Country", "country"
    BindBatchFieldDropdowns ws, "From", "from"
    BindBatchFieldDropdowns ws, "Listed Status", "listed_status"
    BindBatchFieldDropdowns ws, "Tested", "tested"
    BindBatchFieldDropdowns ws, "Bundle Number", "bundle_number"
    BindBatchFieldDropdowns ws, "Stored", "stored"
End Sub
Public Sub BindBatchConsoleDropdowns(ws As Worksheet)
...
Public Sub BindBatchQualityDropdowns(ws As Worksheet)
